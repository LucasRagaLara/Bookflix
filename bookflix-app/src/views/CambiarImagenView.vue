<template>
    <div class="home w-full min-h-screen items-center flex flex-col xl:pt-40 pt-48">
        ...
        <div class="flex items-start justify-center w-full lg:mx-0 gap-x-20 gap-y-10 flex-wrap">
            <div class="flex flex-col xl:w-[12%] xl:mb-12 mb-6 md:w-[40%]" v-for="(imagen_sel, indice) in imagenes_perfiles" :key="imagen_sel.id">
                <h1 v-if="indice % 6 == 0" class="font-bold absolute">{{ obtenerTitulo(indice) }}</h1>
                <!-- <div v-if="indice % 6 == 0" class="xl:mb-10 mb-4 md:mb-0"></div> -->
                <div class="flex items-center justify-end flex-col w-full h-[13rem] 2xl:mt-10 2xl:min-h-[17rem]">
                    <img 
                        :src="imagen_sel.imagen" 
                        class="2xl:min-w-64 2xl:min-h-64 lg:w-32 lg:h-32 w-36 h-36 object-cover cursor-pointer transform transition-transform duration-300 hover:scale-110 hover:border-[#C12C38] hover:border-2 hover:shadow-lg rounded-full"
        ...
        <form class="absolute top-10 lg:right-20 flex flex-col items-end justify-end right-5" @submit.prevent="volverAtras">
                <button type="submit" class="bg-[#C12C38] py-2 rounded-lg transition-all duration-300 ease-in-out transform hover:scale-110 items-center justify-center flex px-5" >CANCELAR</button>
                <div class="flex items-center justify-end mt-10">
                    <h1 class="text-2xl mr-2 lg:mr-5">{{perfilEditando ? perfilEditando.nombre : nuevoPerfil.nombre}}</h1>
